...
class CustomImputer(BaseEstimator, TransformerMixin):
    #Imputing missing values in our data
    def __init__(self):
        print('Customer Imputer initialized')
...
l        garage_missing_none = ['GarageType', 'GarageFinish', 'GarageQual', 
                               'GarageCond']
...
class OrdinalTransformer(BaseEstimator, TransformerMixin):
    #Encoding/Transforming all Ordinal features
...
class FeatureCreator(BaseEstimator, TransformerMixin):
    #Creating new features and appending them to our data
    def __init__(self): # no *args or **kargs
...
class PolynomialFeatureCreator(BaseEstimator, TransformerMixin):
    #Creating polynomial features out of top 10 features correlated the most with our target
    def __init__(self): # no *args or **kargs
        print('Feature Pipeline!')
...
class OutlierPruner(BaseEstimator, TransformerMixin):
    ...
    #Modifying outliers as noted by paper
...
class PriceSplitter(BaseEstimator, TransformerMixin):
    #Separating ID/Price from features
    def __init__(self,train_data): # no *args or **kargs
        self.train = train_data
...
class FeatureSelector(BaseEstimator, TransformerMixin):
    #Selecting all features whose correlation coefficient is above some specified value
...
class DropHighCorr(BaseEstimator, TransformerMixin):
   #From the previous list of features, we remove features that are highly correlated with each other
...
class FeatureTransformer(BaseEstimator, TransformerMixin):
    #Applying various transformations to improve distributions
    def __init__(self): # no *args or **kargs
